...
const CoinFlip = () => (
  <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 px-4">
    <div className="bg-white rounded-2xl p-8 shadow-2xl flex flex-col items-center max-w-sm w-full">
      ...
      <p className="mt-4 text-base font-semibold text-blue-900">Processing Submission...</p>
    </div>
  </div>
);
  ...
  <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
    <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full">
      <div className="p-6 text-center">
        ...
        <h2 className="text-2xl font-bold text-blue-900 mb-2">Payment Submitted Successfully</h2>
        <p className="text-sm text-gray-600 mb-4">Confirmation mail will be received within 48 hours</p>
  ...
  <div className="flex items-center justify-between mb-6 overflow-x-auto pb-2">
    {Array.from({ length: totalSteps }).map((_, i) => (
      <div key={i} className="flex items-center flex-shrink-0">
        ...
        {i < totalSteps - 1 && <div className={`w-6 sm:w-8 md:w-12 h-0.5 mx-1 transition-all ${i < currentStep ? "bg-blue-900" : "bg-gray-300"}`}></div>}
      </div>
    ))}
  </div>
);
...
const CodeFusionPaymentForm = () => {
  ...
  const collegeOptions = [
    "KL University – Vaddeswaram", "SRM University AP – Amaravati", "VIT-AP University – Amaravati",
    "Velagapudi Ramakrishna Siddhartha Engineering College", "Siddhartha Academy of Higher Education",
    "Prasad V Potluri Siddhartha Institute of Technology", "Gudlavalleru Engineering College",
    "Dhanekula Institute of Engineering & Technology", "Andhra Loyola College", "KBN College",
    "NRI Institute of Technology", "JNTUK Kakinada", "Sri Vishnu Engineering College for Women",
    "Sir C. R. Reddy College of Engineering", "Gayatri Vidya Parishad College of Engineering",
    "Lakireddy Bali Reddy College of Engineering", "PACE Institute of Technology",
    "RVR & JC College of Engineering", "Vignan's Foundation for Science & Technology",
    "Vasireddy Venkatadri Institute of Technology", "Bapatla Engineering College", "Others"
  ];

  const [formData, setFormData] = useState({
    teamId: "", teamName: "", problemDomain: "",
    leaderName: "", leaderEmail: "", leaderPhone: "", leaderCollege: "", leaderYear: "",
    leaderPaymentProof: null, leaderTransactionId: "",
    member2Name: "", member2Phone: "", member2College: "", member2Email: "",
    member2PaymentProof: null, member2TransactionId: "",
    member3Name: "", member3Phone: "", member3College: "", member3Email: "",
    member3PaymentProof: null, member3TransactionId: "",
    member4Name: "", member4Phone: "", member4College: "", member4Email: "",
    member4PaymentProof: null, member4TransactionId: "",
  });
  ...
  const handleFileChange = (e, fieldName) => {
    ...
    if (file) {
      if (!file.type.startsWith("image/")) {
        setErrors((prev) => ({ ...prev, [fieldName]: "Only image files allowed (JPG, PNG, etc.)" }));
        return;
      }
  ...
  const validateLeader = () => {
    ...
    else if (!validatePhone(formData.leaderPhone)) newErrors.leaderPhone = "Phone must be 10 digits";
  ...
  const validateMember = (memberNum) => {
    ...
    if (!formData[`${prefix}Phone`]?.trim()) newErrors[`${prefix}Phone`] = "Phone is required";
    else if (!validatePhone(formData[`${prefix}Phone`])) newErrors[`${prefix}Phone`] = "Phone must be 10 digits";
    ...
    else if (!validateEmail(formData[`${prefix}Email`])) newErrors[`${prefix}Email`] = "Invalid email";
  ...
  const handleFinalSubmit = async () => {
    ...
    try {
      ...
      formDataToSend.append("member2PaymentProof", formData.member2PaymentProof);
      ...
      formDataToSend.append("member3PaymentProof", formData.member3PaymentProof);
      ...
      if (teamSize === 4) {
        ...
        formDataToSend.append("member4PaymentProof", formData.member4PaymentProof);
      }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white px-4 py-8 font-sans">
      {isLoading && <CoinFlip />}
      {isVerifying && <VerificationLoadingModal />}
      {showSuccessModal && <SuccessModal registrationId={registrationId} onClose={() => { setShowSuccessModal(false); window.scrollTo({ top: 0, behavior: "smooth" }); }} />}
      {showErrorModal && <ErrorModal message={apiError} onClose={() => setShowErrorModal(false)} />}
      {showConfirmModal && <ConfirmationModal onConfirm={handleFinalSubmit} onCancel={() => setShowConfirmModal(false)} />}

      <div className="max-w-3xl mx-auto mb-6">
        <button onClick={() => step === 0 ? window.history.back() : setStep(step - 1)} className="flex items-center gap-2 text-blue-900 mb-4 font-semibold text-sm hover:text-purple-600 transition-colors">
          <ArrowLeft size={18} />
          <span>Back</span>
        </button>
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="bg-white border-2 border-blue-900/10 rounded-xl shadow-lg p-4 sm:p-6 md:p-8">
          {step === 0 && (
            <div>
              ...
              <div className="mb-5">
                ...
                <input type="text" name="teamId" value={formData.teamId} onChange={handleInputChange}
                  className={`w-full h-11 px-4 border-2 rounded-lg outline-none transition-all text-sm ${errors.teamId ? "border-red-500" : "border-blue-900/20 focus:border-blue-900"}`}
                  placeholder="Enter your team ID" />
                {errors.teamId && <p className="text-red-500 text-xs mt-1 flex items-center gap-1"><AlertCircle size={12} />{errors.teamId}</p>}
              </div>
              <button onClick={handleTeamVerification} disabled={isVerifying}
          ...
          {step === 1 && <TeamDetailsSection formData={formData} handleInputChange={handleInputChange} errors={errors} teamSize={teamSize} setTeamSize={setTeamSize} setErrors={setErrors} onNext={handleNextStep} onPrevious={() => setStep(0)} />}
          {step === 2 && <LeaderDetailsSection formData={formData} handleInputChange={handleInputChange} handleFileChange={handleFileChange} errors={errors} collegeOptions={collegeOptions} onNext={handleNextStep} onPrevious={() => setStep(1)} />}
          {step === 3 && <MemberDetailsSection memberNum={2} formData={formData} handleInputChange={handleInputChange} handleFileChange={handleFileChange} errors={errors} collegeOptions={collegeOptions} onNext={handleNextStep} onPrevious={() => setStep(2)} />}
          {step === 4 && teamSize >= 3 && <MemberDetailsSection memberNum={3} formData={formData} handleInputChange={handleInputChange} handleFileChange={handleFileChange} errors={errors} collegeOptions={collegeOptions} onNext={handleNextStep} onPrevious={() => setStep(3)} teamSize={teamSize} />}
          {step === 5 && teamSize === 4 && <MemberDetailsSection memberNum={4} formData={formData} handleInputChange={handleInputChange} handleFileChange={handleFileChange} errors={errors} collegeOptions={collegeOptions} onNext={handleNextStep} onPrevious={() => setStep(4)} />}
          {step === totalSteps && <FinalReviewSection formData={formData} teamSize={teamSize} onConfirm={() => setShowConfirmModal(true)} onPrevious={() => setStep(step - 1)} />}
        </div>
      </div>
    </div>
  );
...
const TeamDetailsSection = ({ formData, teamSize, setTeamSize, setErrors, onNext, onPrevious, errors }) => (
  <div>
    <div className="flex items-center gap-2 mb-5">
      <div className="w-9 h-9 rounded-full bg-blue-900 flex items-center justify-center"><Users size={18} className="text-white" /></div>
    ...
    <div className="space-y-4 mb-5">
      <div>
        ...
        <input type="text" value={formData.teamName} disabled
        ...
        <p className="text-xs text-gray-500 mt-1">Auto-filled from team verification</p>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-bold text-blue-900 mb-1.5">Team Size *</label>
          <select value={teamSize} onChange={(e) => { setTeamSize(parseInt(e.target.value)); setErrors((prev) => ({ ...prev, teamSize: "" })); }}
            className={`w-full h-11 px-4 border-2 rounded-lg outline-none text-sm cursor-pointer transition-all ${errors.teamSize ? "border-red-500" : "border-blue-900/20 focus:border-blue-900"}`}>
        ...
        <div>
          <label className="block text-sm font-bold text-blue-900 mb-1.5">Problem Statement *</label>
          <input type="text" value={formData.problemDomain} disabled
            className="w-full h-11 px-4 border-2 rounded-lg outline-none text-sm bg-gray-100 border-blue-900/20 text-gray-700 cursor-not-allowed"
            placeholder="Auto-filled from verification" />
          <p className="text-xs text-gray-500 mt-1">Auto-filled from team verification</p>
        </div>
...
const LeaderDetailsSection = ({ formData, handleInputChange, handleFileChange, errors, collegeOptions, onNext, onPrevious }) => (
  <div>
    <div className="flex items-center gap-2 mb-5">
      <div className="w-9 h-9 rounded-full bg-blue-900 flex items-center justify-center"><GraduationCap size={18} className="text-white" /></div>
    ...
    <div className="space-y-4 mb-6">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <input type="text" name="leaderName" value={formData.leaderName} onChange={handleInputChange}
            className={`w-full h-11 px-4 border-2 rounded-lg outline-none transition-all text-sm ${errors.leaderName ? "border-red-500" : "border-blue-900/20 focus:border-blue-900"}`}
        ...
        <div>
          ...
          <input type="email" name="leaderEmail" value={formData.leaderEmail} onChange={handleInputChange}
            className={`w-full h-11 px-4 border-2 rounded-lg outline-none transition-all text-sm ${errors.leaderEmail ? "border-red-500" : "border-blue-900/20 focus:border-blue-900"}`}
        ...
        <div>
          ...
          <input type="tel" name="leaderPhone" value={formData.leaderPhone} onChange={handleInputChange}
            className={`w-full h-11 px-4 border-2 rounded-lg outline-none transition-all text-sm ${errors.leaderPhone ? "border-red-500" : "border-blue-900/20 focus:border-blue-900"}`}
        ...
        <div>
          ...
          <select name="leaderCollege" value={formData.leaderCollege} onChange={handleInputChange}
            className={`w-full h-11 px-4 border-2 rounded-lg outline-none text-sm cursor-pointer transition-all ${errors.leaderCollege ? "border-red-500" : "border-blue-900/20 focus:border-blue-900"}`}>
        ...
        <div>
          ...
          <select name="leaderYear" value={formData.leaderYear} onChange={handleInputChange}
            className={`w-full h-11 px-4 border-2 rounded-lg outline-none text-sm cursor-pointer transition-all ${errors.leaderYear ? "border-red-500" : "border-blue-900/20 focus:border-blue-900"}`}>
    ...
    <div className="bg-gradient-to-br from-blue-900/5 to-purple-600/5 border-2 border-blue-900/10 rounded-lg p-4 sm:p-5 mb-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
        <div>
          ...
          <a href="https://vrsec.campx.in/sahe/ums/public/form/695cd13508b4c95ea665f366" target="_blank" rel="noopener noreferrer"
        ...
        <div>
          ...
          <input type="file" accept="image/*" onChange={(e) => handleFileChange(e, "leaderPaymentProof")}
            className={`w-full px-3 py-2.5 border-2 rounded-lg outline-none transition-all cursor-pointer text-sm ${errors.leaderPaymentProof ? "border-red-500" : "border-blue-900/20 focus:border-blue-900"}`} />
...
const MemberDetailsSection = ({ memberNum, formData, handleInputChange, handleFileChange, errors, collegeOptions, onNext, onPrevious, teamSize }) => {
  const prefix = `member${memberNum}`;
  ...
  return (
    <div>
      <div className="flex items-center gap-2 mb-5">
        <div className="w-9 h-9 rounded-full bg-blue-900 flex items-center justify-center"><Users size={18} className="text-white" /></div>
        <h2 className="text-lg sm:text-xl font-bold text-blue-900">Member {memberNum} Details</h2>
      </div>
      <div className="space-y-4 mb-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-bold text-blue-900 mb-1.5">Full Name *</label>
            <input type="text" name={`${prefix}Name`} value={formData[`${prefix}Name`]} onChange={handleInputChange}
              className={`w-full h-11 px-4 border-2 rounded-lg outline-none transition-all text-sm ${errors[`${prefix}Name`] ? "border-red-500" : "border-blue-900/20 focus:border-blue-900"}`}
              placeholder="Enter full name" />
          ...
          <div>
            ...
            <input type="tel" name={`${prefix}Phone`} value={formData[`${prefix}Phone`]} onChange={handleInputChange}
              className={`w-full h-11 px-4 border-2 rounded-lg outline-none transition-all text-sm ${errors[`${prefix}Phone`] ? "border-red-500" : "border-blue-900/20 focus:border-blue-900"}`}
              placeholder="10-digit phone number" maxLength="10" />
          ...
          <div>
            <label className="block text-sm font-bold text-blue-900 mb-1.5">College/University *</label>
            <select name={`${prefix}College`} value={formData[`${prefix}College`]} onChange={handleInputChange}
              className={`w-full h-11 px-4 border-2 rounded-lg outline-none text-sm cursor-pointer transition-all ${errors[`${prefix}College`] ? "border-red-500" : "border-blue-900/20 focus:border-blue-900"}`}>
              <option value="">Select College</option>
              {collegeOptions.map((college) => <option key={college} value={college}>{college}</option>)}
            </select>
          ...
          <div>
            <label className="block text-sm font-bold text-blue-900 mb-1.5">Email Address *</label>
            <input type="email" name={`${prefix}Email`} value={formData[`${prefix}Email`]} onChange={handleInputChange}
              className={`w-full h-11 px-4 border-2 rounded-lg outline-none transition-all text-sm ${errors[`${prefix}Email`] ? "border-red-500" : "border-blue-900/20 focus:border-blue-900"}`}
              placeholder="student@email.com" />
      ...
      <div className="bg-gradient-to-br from-blue-900/5 to-purple-600/5 border-2 border-blue-900/10 rounded-lg p-4 sm:p-5 mb-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
          ...
          <div>
            <label className="block text-sm font-bold text-blue-900 mb-2">Upload Payment Proof *</label>
            <input type="file" accept="image/*" onChange={(e) => handleFileChange(e, `${prefix}PaymentProof`)}
              className={`w-full px-3 py-2.5 border-2 rounded-lg outline-none transition-all cursor-pointer text-sm ${errors[`${prefix}PaymentProof`] ? "border-red-500" : "border-blue-900/20 focus:border-blue-900"}`} />
            {formData[`${prefix}PaymentProof`] && <p className="text-green-600 text-xs mt-1 flex items-center gap-1"><Check size={12} />{formData[`${prefix}PaymentProof`].name?.substring(0, 25)}...</p>}
      ...
      <div className="flex gap-3">
        <button onClick={onPrevious} className="flex-1 h-11 text-sm font-bold text-blue-900 bg-gray-200 rounded-lg transition-all hover:bg-gray-300">Back</button>
        <button onClick={onNext} className="flex-1 h-11 text-sm font-bold text-white bg-gradient-to-r from-purple-600 to-purple-800 rounded-lg transition-all hover:from-purple-700 hover:to-purple-900 flex items-center justify-center gap-2">
          {isLastMember ? "Review & Submit" : "Next Member"} <ChevronRight size={16} />
        </button>
      </div>
    </div>
  );
...
const FinalReviewSection = ({ formData, teamSize, onConfirm, onPrevious }) => {
  ...
  return (
    <div>
      <div className="flex items-center gap-2 mb-5">
        <div className="w-9 h-9 rounded-full bg-blue-900 flex items-center justify-center"><CheckCircle size={18} className="text-white" /></div>
        <h2 className="text-lg sm:text-xl font-bold text-blue-900">Final Review</h2>
      </div>
      <div className="bg-gradient-to-br from-blue-900/5 to-purple-600/5 border-2 border-blue-900/10 rounded-lg p-4 sm:p-5 mb-6">
        <h3 className="text-sm font-bold text-blue-900 mb-4 flex items-center gap-2"><Info size={16} />Payment Status</h3>
        {[["Team Leader", "leaderPaymentProof"], ["Member 2", "member2PaymentProof"], ["Member 3", "member3PaymentProof"], ...(teamSize === 4 ? [["Member 4", "member4PaymentProof"]] : [])].map(([label, key]) => (
          <div key={key} className="flex items-center justify-between p-3 bg-white rounded-lg shadow-sm mb-2">
            <span className="font-semibold text-sm">{label}</span>
            {isUploaded(key) ? (
              <span className="flex items-center gap-1 text-green-600 font-bold text-xs"><CheckCircle size={16} />Uploaded</span>
            ) : (
              <span className="flex items-center gap-1 text-red-600 font-bold text-xs"><X size={16} />Pending</span>
            )}
          </div>
        ))}
      </div>
      {!allPaymentsComplete() && (
        <div className="bg-amber-50 border-2 border-amber-500 rounded-lg p-3 mb-6 flex items-start gap-2">
          ...
          <div>
            <p className="font-bold text-amber-900 text-sm">Incomplete Payment</p>
            <p className="text-xs text-amber-800">Please upload payment proofs for all team members.</p>
          </div>
        </div>
      )}
      <div className="flex gap-3">
        <button onClick={onPrevious} className="flex-1 h-11 text-sm font-bold text-blue-900 bg-gray-200 rounded-lg transition-all hover:bg-gray-300">Back</button>
        <button onClick={onConfirm} disabled={!allPaymentsComplete()}
